...
func TestChartPoissonLambdas2(t *testing.T) {
	...
	for _, hashratePortion := range hashrateProportions {
		for interval := float64(0); interval <= intervalSamples; interval++ {
			...
			poissonDist = distuv.Poisson{
				Lambda: interval * networkLambda,
				Src:    exprand.NewSource(uint64(time.Now().UnixNano())),
			}
			data2 = append(data2, plotter.XY{X: interval, Y: (1 - poissonDist.CDF(0)) * hashratePortion})
		}
	...
	if err := p.Save(800, 600, "poisson_2_lambda.png"); err != nil {
		t.Fatal(err)
	}
}

/*
From this, we can see that the second (Blue200) model is not adequate as a model of the world.
Under it, the 25% hashrate miner's probability of an event (winning a block) does not exceed 0.25,
when, in fact, we know that it should approach 1 eventually.
*/
