...
func TestChartPoissonLambdas4(t *testing.T) {
	...
	for _, hashratePortion := range hashrateProportions {
		for interval := float64(1); interval <= intervalSamples; interval++ {
		...
		p.Add(line)
		p.Legend.Add(fmt.Sprintf("Poisson(1/13 * %0.2f).CDF(1))", hashratePortion), line)
	...
	if err := p.Save(800, 600, "poisson_4_lambda.png"); err != nil {
		t.Fatal(err)
	}
}
