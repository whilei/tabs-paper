...
It is a representative aggregate of all transaction sender accounts (and the miner) for a blockchain of some length.
...
A private messaging service, like e-mail, could be used to send transaction information to miners.
In this context, the sender trusts a miner to handle a transaction privately
until their next block composition.
But this example clearly depends on a unilateral trust between the transactor and their miner of choice.
...
By adding a demand on some chain state at a block $H_n$, the transaction expresses
