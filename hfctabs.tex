...
\small{A Canonical-Arbitration Algorithm Condition Program\
for Modified-GHOST PoW Blockchain Protocols}}
...
Ethereum.
%Ethereum uses a Proof of Work protocol ("Ethash") underpinning a "Modified
%GHOST" canonical-arbitration algorithm.
...
% TODO cite the Modified GHOST reference.

Our proposed design is an augmentation.
It assumes and depends on the existence of other (canonical-arbitration)
algorithmic conditions. The necessary general characteristics of such
pre-existing canonical-arbitration algorithm conditions are not defined nor
assumed in this paper beyond what may be generally assumed for PoW protocols.

%Should we name the proposal...'TABS'?
...
We detail the associated theoretical additional risk (cost), which we show to
be practically nominal.
We note also that implementation of this design is feasible for Ethereum's
primary client \textit{go-ethereum} (\texttt{geth}) and probably feasible for
most existing PoW clients.
%This security by augmenting the protocol's canonical-arbitration algorithm
%with a condition on a measure of necessary majority hashrate with a demand on
%activation of sufficient on-chain capital.
