...
\subsection{\small{Segment-Specific Transactions: Transactions with Ancestor Hash Appended}}
...
% In practice, this could be worked around, but without guarantees.
There are existing ways that this is practically possible, though limited in theory compared with
the proposal outlined below.
Email or any other private messaging service could be used to send miners
transaction information, which the sender would assume would be processed privately by the miner
upon their winning of a next block.
However, there could be no trustless guarantee that the miner would actually keep the transaction
secret before including it in a block; it could be resold, publicized, ignored;
all at the miner's discretion.

The problem of enabling transactions to define chain-exclusivity (ie. segment-specificity)
as a rule that would enforceable on-chain is addressed here.
