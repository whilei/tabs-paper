...
the same canon-arbitration score, and yet it does not appear to be the case that block heights are intentionally
mined for the duration of their competitive interval for some block height,
we must infer that an incentive exists for the miner to mine the \textit{next} block;
to build from the latest best available block to them, rather than pursue a later-occurring but equivalent (or better)
canon-scoring block.
...
%        Cases where TABS remains constant between parent and child blocks are not represented
%        because these cases are taken for granted to be extremely rare.
    }
...
\begin{figure}[tph]
    ...
    \caption{
        ...
        indicating that it is probably more profitable for them to begin a new round.
    }
