...
\begin{figure}[H]
\begin{center}
\includegraphics[page=1,width=1.0\textwidth,height = 0.3
\textheight]{img/POINTABS_Transaction Pools.pdf}
\end{center}
...
$TABS(B_{i}, B_{i-1})$ is a weighted average of $TAB(B_{i})$ and its value on the parent block with weighting parameter $\alpha \in (0,1)$. We prove by induction that, for any two successive blocks, $POINTABS$ is maximized by setting the $TAB$ of the child block higher. Consider block \#2. 

$TABS(B_{2}, B_{1}) = \alpha * TAB(B_{2}) + (1 - \alpha)*TAB(B_{1})$, noting that $TABS(B_{1},B_{0})  = TAB(B_{1})$.

The $POINTABS$ score of successive blocks \#1 and 2 is

$TABS(B_{2}, B_{1}) + TABS(B_{1}, B_{0}) = \alpha*TAB(B_{2}) + (2 - \alpha)*\alpha*TAB(B_{1})$.

The marginal impact is recovered by taking the derivatives of the successive $TABS$ terms.

$\frac{d}{d TAB(B_{2})} = \alpha$

$\frac{d}{d TAB(B_{1})} = (2 - \alpha)*\alpha$

Since $(2 - \alpha) > 1$ it follows that an increment of $TAB(B_{1})$ has a greater impact on the $TABS$ score than does $TAB(B_{2})$. The $POINTABS$ score of blocks \#1 and 2 is maximized by maximizing $TAB(B_{1})$.

Next, consider any block $i$.

$TABS(B_{i}, B_{i-1}) = \alpha * TAB(B_{i}) + (1 - \alpha)*TABS(B_{i-1}, B_{i-2}) = \alpha * TAB(B_{i}) + (1 - \alpha)*\big\{\alpha * TAB(B_{i-1}) + (1 - \alpha)*[\alpha * TAB(B_{i-2}) + (1 - \alpha)*TABS(B_{i-2}, B_{i-3})]\big\}$

The $POINTABS$ score of two successive blocks $\{B_{i}, B_{i+1}\}$ (ignoring the constant mining puzzle difficulty) is

$TABS(B_{i+1}, B_{i}) + TABS(B_{i}, B_{i-1}) = \underset{\text{current blocks}}{\underbrace{\alpha*TAB(B_{i+1}) + (2 - \alpha)*\alpha*TAB(B_{i})}} + \\
\underset{\text{parent block}}{\underbrace{(2 -\alpha)*(1 - \alpha)*TABS(B_{i-1}, B_{i-2})}}$

The marginal impact is recovered by taking the derivatives of the successive $TABS$ terms.

$\frac{d}{d TAB(B_{i})} = \alpha$

$\frac{d}{d TAB(B_{i-1})} = (2 - \alpha)*\alpha$

Since $(2 - \alpha) > 1$ it follows that an increment of $TAB(B_{i-1})$ has a greater impact on the $TABS$ score than does $TAB(B_{i})$. The $POINTABS$ score of blocks \#1 and 2 is maximized by maximizing $TAB(B_{i})$. 



Therefore, starting at the genesis block \#1, the $POINTABS$ score of a sequence of 

. With a fixed pool of transactions this can be achieved by maximizing $TAB(B_{i})$ for each block.
\end{proof}
\end{comment}
...
Protocol. \\~\\
%% $\mathrm{Canonical-Arbitration Algorithm}$ & A program of comparative
...
\begin{figure}[tph]
    ...
    \centering
    \includegraphics[width=1.0\textwidth]{go-poisson-collision/out/vis_poisson_cdf_next_in_interval.png}
    \caption{
...
\begin{figure}[tph]
    ...
    \centering
    \includegraphics[width=1.0\textwidth]{go-poisson-collision/out/vis_poisson_samples_events_86400.png}
    \caption{
...
\begin{figure}[tph]
    ...
    \centering
\includegraphics[width=1.0\textwidth]{go-poisson-collision/out/vis_poisson_samples_eventintervals_hist.png}
    \caption{
...
%\begin{figure}[tph]
%    \centering
%    \includegraphics[width=1.0\textwidth]{go-poisson-collision/out/vis_poisson_samples_events_8640000.png}
%    \caption{
...
\begin{figure}[tph]
    \centering
\includegraphics[width=1.0\textwidth]{go-poisson-collision/out/vis_poisson_samples_eventintervals_latencynaive_hist.png}
    \caption{
...
\includegraphics[width=1.0\textwidth]{go-poisson-collision/out/vis_poisson_samples_eventintervals_latencysamesame_hist.png}
    \caption{
      Procedurally generated events $k \geq 1$ interval samples from a Poisson distribution
with $\lambda = \frac{1}{14}$, with a sample size of 86400.
...
\begin{figure}[tph]
    ...
    \includegraphics[width=1.0\textwidth]{go-poisson-collision/out/vis_poisson_samples_eventintervals_latencysamesamestrat_hist.png}
    \caption{
        Procedurally generated events $k \geq 1$ interval samples from a Poisson distribution
        with $\lambda = \frac{1}{14}$, with a sample size of 86400.
